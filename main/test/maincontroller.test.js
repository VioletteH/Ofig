...
describe("mainController", () => {
	...
	describe("homePage", () => {
		test('should render "accueil" with all figurines', async () => {
			const mockFigurines = [
				{ id: 1, name: "Figurine 1" },
				{ id: 2, name: "Figurine 2" },
			];
			...
			expect(mockResponse.render).toHaveBeenCalledWith("accueil", {
				figurines: mockFigurines,
			});
		...
		test("should handle errors and send 500 status", async () => {
			...
			const consoleErrorSpy = jest
				.spyOn(console, "error")
				.mockImplementation(() => {}); // Supprimer les logs d'erreur pendant le test
	...
	describe("articlePage", () => {
		test('should render "article" with figurine and reviews', async () => {
			...
			expect(figurineDataMapper.getOneFigurine).toHaveBeenCalledWith(
				figurineId,
			);
			expect(figurineDataMapper.getAllReviews).toHaveBeenCalledWith(figurineId);
			expect(mockResponse.render).toHaveBeenCalledWith("article", {
				figurine: mockFigurine,
				reviews: mockReviews,
			});
		});
	...
	describe("search", () => {
		test('should render "article" with figurine and reviews for a search query', async () => {
			...
			expect(figurineDataMapper.getAllReviews).toHaveBeenCalledWith(
				mockFigurine.id,
			);
			expect(mockResponse.render).toHaveBeenCalledWith("article", {
				figurine: mockFigurine,
