...
describe("figurineDataMapper", () => {
	...
	describe("getAllFigurines", () => {
		test("should return all figurines with average note", async () => {
			const mockRows = [
				{ id: 1, name: "Figurine 1", note: 4 },
				{ id: 2, name: "Figurine 2", note: 3.5 },
			];
        ...
        LEFT JOIN review`;
			...
			expect(normalizeSql(actualQuery)).toContain(
				normalizeSql(expectedQueryPart),
			);
		...
		test("should handle database errors", async () => {
			...
			await expect(figurineDataMapper.getAllFigurines()).rejects.toThrow(
	...
	describe("getOneFigurine", () => {
		test("should return a single figurine by ID with average note", async () => {
			...
			expect(client.query).toHaveBeenCalledWith(
				expect.stringContaining("WHERE figurine.id = $1"),
				[1],
			);
	...
	describe("getCategory", () => {
		test("should return figurines by category with average note", async () => {
			const mockFigurines = [
				{ id: 3, name: "Figurine C", category: "CategoryX" },
			];
	...
	describe("countCategory", () => {
		test("should return category counts", async () => {
			const mockCounts = [
				{ category: "CategoryA", count: 5 },
				{ category: "CategoryB", count: 3 },
			];
	...
	describe("search", () => {
		test("should return figurine by name search with average note", async () => {
			...
			expect(client.query).toHaveBeenCalledTimes(1);
			expect(client.query).toHaveBeenCalledWith(
				expect.stringContaining("WHERE figurine.name ILIKE $1"),
				["%searched%"],
			);
			expect(figurine).toEqual(mockFigurine);
		});
